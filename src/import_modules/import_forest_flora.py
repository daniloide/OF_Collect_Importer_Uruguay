...
# -*- coding: utf-8 -*-s
"""
This script provides the functionality to import_modules the forest flora information from the DGF data either in the 2015 or
...
def import_forest_flora(survey, infile, format, verbose=None):
    ...
    if format == '2010':
        import_flora_soto_2010.import_flora_soto_2010(survey, infile)
    elif format == '2015':
        import_flora_soto_2015.import_flora_soto_2015(survey, infile)
    else:
    ...
    logging.info(info_msg)
    now = datetime.datetime.now()
    info_msg = ' Finished the import_modules of forest flora information dataset {date}'. \
        format(date=now.strftime("%Y-%m-%d %H:%M"))
    logging.info(info_msg)
...
if __name__ == "__main__":
    ...
    import_forest_flora(survey,  InFileNamePlots, format, verbose)
    survey.export_flora_soto(OutFileName)